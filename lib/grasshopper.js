...
function Shot(position, direction, playerId) {
  ...
  this.flyAndKill = function() {
    if (life > 0) {
      ...
      this.position = this.position.add(ahead.mul(WorldDefs.gameTick()).mul(WorldDefs.speed));
      this.life--;
    } else {
      delete getGame.world.shotList[this.playerId];
      return;
    };
  }
...
function Game() {
  ...
  this.fireShot = function(options) {
    var shooter = options.player;

    if (this.world.shotList[shooter.id]) { 
      return;
    } else {
      this.world.shotList[shooter.id] = new Shot(shooter.position, shooter.direction, shooter.id);
    };
  };
  ...
  this.turn = function() {
    ...
    Object.keys(this.world.shotList).forEach(function(shot) {
      shot.flyAndKill();
    });
...
function World() {
  ...
  this.detectCollisions = function() {
    var obstacles = this.players.concat(Object.keys(this.shotList));
